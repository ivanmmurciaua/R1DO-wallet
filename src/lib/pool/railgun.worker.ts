...
// ── Runtime config injected by the MAIN thread at bootEngine ──────────────────
// A Web Worker has no window/localStorage, so it cannot read the active network
// or the RPC list itself: both live behind window.localStorage on the main
// thread and are frozen at import there. Main resolves them and passes them in;
// everything else derives from NETWORK_CONFIG, a static SDK constant that is
// worker-safe. `./config` holds the shared shape + the main-side resolver.
export type PoolConfig = { networkName: NetworkName; rpcUrls: string[] };

// Representative config shape (all networks share it) — keeps the exact field
// types the code below expects without importing them one by one.
type NetCfg = (typeof NETWORK_CONFIG)[NetworkName.Arbitrum];
let POOL_NETWORK!: NetworkName;
let CHAIN!: NetCfg["chain"];
let POOL_PROXY!: NetCfg["proxyContract"];
let WETH!: NetCfg["baseToken"]["wrappedAddress"];
let RPC_URLS!: string[];
// Primary RPC for the light ethers reads (block number, feeData); Railgun's
// heavy batched scans get the full failover list via loadProvider.
let RPC!: string;
...
function applyConfig(cfg: PoolConfig): void {
  POOL_NETWORK = cfg.networkName;
  RPC_URLS = cfg.rpcUrls;
  RPC = cfg.rpcUrls[0];
  const nc = NETWORK_CONFIG[POOL_NETWORK];
  CHAIN = nc.chain;
  WETH = nc.baseToken.wrappedAddress;
  POOL_PROXY = nc.proxyContract;
}
...
export async function bootEngine(cfg: PoolConfig): Promise<void> {
  applyConfig(cfg); // main injects the network + RPCs it read from localStorage
  if (engineBooted) {
...
export async function createPoolWallet(
  ...
  // Creation block = this wallet's scan-start. The main thread resolves a chosen
  // import/recovery date to a block (it owns blockByTime + localStorage) and
  // passes the number in; without one we assume a genuinely new 0zk with no
  // prior funds and start near the tip. A too-recent start silently hides older
  // shields, which is exactly the bug the date override exists to fix.
  let creationBlock: number;
  if (creationBlockOverride !== undefined) {
    creationBlock = creationBlockOverride;
    console.log(`[pool] creationBlock (override) = ${creationBlock}`);
  } else {
    console.log("[pool] reading current block…");
    const block = await provider().getBlockNumber();
    creationBlock = Math.max(block - 100, 0);
    console.log(`[pool] creationBlock (fresh wallet) = ${creationBlock}`);
  }
...
if (typeof self !== "undefined") {
  // `self` is the global in BOTH a Worker and the main thread, so this POI-noise
  // swallower installs inside the worker where the engine actually runs.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
async function watcherTick(): Promise<void> {
  if (!poolWallet || recoverPending) return;
  await withTimeout(
    syncRailgunTransactionsV2(POOL_NETWORK),
    60_000,
    "syncTxid",
  ).catch(() => {});
  if (recoverPending || !poolWallet) return;
  ...
  if (key && key !== lastGeneratedPOIKey) {
    ...
    if (recoverPending || !poolWallet) return; // skip the ~180s POI burn
    await withTimeout(
...
// Set by recoverPoolBalances so an in-flight watcher tick BAILS at its next await
// instead of burning up to ~4 min on POI generation for a wallet about to be
// deleted (that wait is what made the re-anchor "slow to start").
...
async function clearWalletAndRescan(
  ...
  if (!engineBooted) throw new Error("engine not booted");
  if (!poolWallet || poolWallet.username !== username || !walletEncryptionKey)
...
// freezes (the mobile-freeze fix). The main thread drives it through this surface
// via ./railgun.ts (the thin proxy). Callbacks passed in from main arrive as
// Comlink proxies and are called normally (fire-and-forget across the boundary).
const workerApi = {
  bootEngine,
  isEngineBooted,
  getPoolFees,
